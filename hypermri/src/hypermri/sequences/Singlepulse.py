...
def Singlepulse(path_or_BrukerExp):
    """Wrapper which decides wether to load the single or dualchannel Singlepulse class.
    ...
    if tmp_brukerexp.n_receivers == 2:
        return Singlepulse_DualChannel(path_or_BrukerExp)
    else:
        return Singlepulse_SingleChannel(path_or_BrukerExp)


class Singlepulse_SingleChannel(BaseSpectroscopy):
    ...
    def plot(self, unit="Hz"):
        if self.NR == 1:
            return self.single_spec_plot(self.spec, unit)
        else:
            raise NotImplementedError(
                "Plotting for multpile repetitions not implemented yet."
            )
    ...
    def calculate_T1(
        ...
        spec, fids,complex_spec = self.get_spec(0, 70)
        ...
        print(
            "7T measurement -- FA and TR corrected T1= ",
            np.round(T1, 1),
            "plus minus ",
            np.round(T1error, 1),
            " s",
...
class Singlepulse_DualChannel(BaseSpectroscopy):
    def __init__(self, path_or_BrukerExp):
        """Accepts directory path or BrukerExp object as input."""
        if isinstance(path_or_BrukerExp, BrukerExp):
            path_or_BrukerExp = path_or_BrukerExp.path
