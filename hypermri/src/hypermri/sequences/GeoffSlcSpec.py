...
class GeoffSlcSpec(BaseSpectroscopy):
    def __init__(self, path_or_BrukerExp):
        ...
        self.NSPacks = self.method["PVM_NSPacks"]

        self.rawdata = self.Load_rawdatajob0_file()

        self.spack_length = len(self.rawdata) // self.NSPacks

        self.fids = []
        self.specs = []
        self.complex_specs = []

        for i in range(self.NSPacks):
            rawfid = self.rawdata[self.spack_length * i : self.spack_length * (i + 1)]

            spec, fid, cmplx_spec = self.get_spec(rawfid, LB=5)
